  ...
  return (
    <>
      <nav className="bottom-nav" aria-label="Mobile navigation">
        <div style={{ position: 'absolute', top: 0, height: 2, background: 'var(--gold)', borderRadius: 2, transition: 'all .3s cubic-bezier(.34,1.56,.64,1)', width: '16%', left: `${ITEMS.findIndex(i => i.key === page) * 20}%`, boxShadow: '0 0 8px rgba(255,179,71,.5)', opacity: ITEMS.some(i => i.key === page) ? 1 : 0 }} />
        {ITEMS.map(({ key, label }) => (
          <div
            key={key}
